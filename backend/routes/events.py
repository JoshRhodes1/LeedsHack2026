        ...
        for p_tag in profile_tags:
            for e_tag in e_tags:
                ...
                if score >= 0.85:  # Threshold for recommendation
                    match_score += score
                if score >= 0.35:
                    match_score += score * 0.15  # Partial match contributes less
                if score >= 0.1:
                    match_score += score * 0.05  # Weak match contributes even less
        if match_score > 1.0:
            #print(f"Event '{event['name']}' has a match score of {match_score:.4f} for profile {profile_id}")
